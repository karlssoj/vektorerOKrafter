...
public class GravitationalForce : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //Hämtar en referens till månens spelobjekt
        Moon = GameObject.Find("Moon");
        
        //Hämtar en referens till månens fysikmotor via månens spelobjekt
    ...
    void Update()
    {
        //Beräknr avståndet mellan månen och jorden för varje frame
        float r = Vector2.Distance(transform.position, Moon.transform.position);

        //Beräknar gravitationskraften enligt Newtons gravitationslag. Jordens och månens
        //massor är definierade i månes och jordens Rigidbody2D-komponenter
        float F = G*((MoonPhysics.mass * EarthPhysics.mass)/(r*r));

        //Skapr en vektor Direction för att beskriva riktningen från månen till jorden
        ...
        //Då får vi en kraft F som är riktad från månen till jorden. 
        Direction.Normalize();
        Vector2 Force = Direction * F;

        //Utsätter månen för gravitationskraften
